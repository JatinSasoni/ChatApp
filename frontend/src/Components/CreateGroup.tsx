...
const CreateGroup = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (groupProfilePic) {
      profilePhoto = await readAsDataURL(groupProfilePic);
    }
    setError("");
  ...
  return (
    <section className="flex max-sm:flex-col">
      <Navbar />
      <section className="w-full flex sm:justify-center sm:items-center  min-h-screen bg-gray-100">
        <form
          className="w-full max-sm:pt-10 max-w-md mx-auto p-6 bg-white sm:border-1 sm:border-slate-400 rounded-xl shadow-lg space-y-6"
